...
_CANDIDATE_SHADOW_OUTCOME_FIELDS = (
    ...
    # Renamed in PR-AD commit 1c per Codex: the candidate outcome is
    # priced from mid quotes only — bid/ask-adjusted execution returns
    # are NOT computed for the candidate path. The field name now
    # embeds the pricing grade so downstream aggregators cannot quietly
    # call this an "execution return."
    "mid_realized_return_pct",
    "iv_change_front", "iv_change_back",
)


# ── Sample provenance taxonomy (PR-AD commits 1b/1c, Codex review) ────────
#
...
# the historical replay after PR-AD merges produces evidence that is
# still in-sample — the data was generated by the same code path used
# to discover the rule.
#
# DEFENSIVE INVARIANT (enforced by the tag_* helpers below, plus
# regression tests): the only place that may assign
# SAMPLE_PROVENANCE_HISTORICAL_REPLAY is _simulate_pre_earnings_calendar
# _trade (or future historical-only callers). The only place that may
# assign SAMPLE_PROVENANCE_FORWARD_POST_FREEZE is tag_live_forward_
# observation, called exclusively from the live API path. Casual
# assignment from CLI flags, scripts, or downstream code is forbidden —
# otherwise someone could run historical replay with a "set provenance
# to forward" flag and recreate the leakage failure mode.
#
# Values:
SAMPLE_PROVENANCE_HISTORICAL_REPLAY = "historical_replay_in_sample_or_research"
#   Output of scripts/replay_*.py or _simulate_pre_earnings_calendar_trade.
#   ALWAYS in-sample for the +14d rule (the rule was discovered on this
#   exact replay pipeline). NEVER promotion-eligible.
SAMPLE_PROVENANCE_FORWARD_POST_FREEZE = "forward_post_freeze"
#   Recommendation produced by the live engine in real time, against
#   a live chain, for an upcoming earnings event. Promotion-eligible
#   IFF created_at is after the PR-AD merge commit. The "freeze" in
#   the name refers to the rule being frozen at PR-AD; only post-freeze
#   forward observations count.
#
#   ASSIGNMENT IS RESTRICTED — only `tag_live_forward_observation`
#   may write this value. Any other code path that assigns it directly
#   is a research-leakage hazard and will be flagged by the regression
#   test in tests/unit/test_web/test_calendar_dual_picker.py.
SAMPLE_PROVENANCE_HISTORICAL_HOLDOUT_PREREGISTERED = "historical_holdout_preregistered"
#   Escape hatch for a holdout set that was frozen BEFORE any analysis.
#
#   NOT USABLE TODAY. A string label is not enough — when this becomes
#   a real feature, it MUST require a frozen manifest artifact:
#     - explicit list of symbols + date ranges in the holdout universe
#     - created_at timestamp predating the rule discovery
#     - content hash of the manifest
#     - author + reason
#   Until that infrastructure exists, this constant is reserved but
#   never auto-assigned. The promotion criteria honor it iff the
#   record carries a holdout_manifest_hash field that resolves to a
#   pre-registered artifact (verification will happen in a future PR).
SAMPLE_PROVENANCE_UNKNOWN = "unknown"
#   Default. Never promotion-eligible. Used when the caller did not
#   explicitly tag provenance — fail-closed. Diagnostic aggregators
#   SHOULD still surface UNKNOWN counts to maintain observability,
#   but they MUST NOT be folded into promotion-eligible totals.
...
# block is research-grade evidence only. Per Codex hard requirement.
